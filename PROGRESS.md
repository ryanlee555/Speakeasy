...
- Next Supabase step: decide on Storage bucket structure (per-user folder path, RLS policy shape) for the video-upload follow-up; verify end-to-end with a real signed-up user + a recorded clip landing in the `recordings` table (**now genuinely testable — the insert exists as of 2026-08-13; before that it never would have worked**)
